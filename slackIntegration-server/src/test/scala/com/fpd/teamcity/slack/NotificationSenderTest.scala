...
import com.fpd.teamcity.slack.ConfigManager.{
  BuildSetting,
  BuildSettingFlag,
  Config
}
...
class NotificationSenderTest
  ...
  "NotificationSender.prepareSettings" should "return setting if build success" in new Context {
    ...
    sender
      .prepareSettings(build, Set(BuildSettingFlag.success))
      .toSet shouldEqual Set(setting)
  }

  "NotificationSender.prepareSettings" should "not return setting if build success" in new Context {
    def settingFlags =
      Set(BuildSettingFlag.success, BuildSettingFlag.failureToSuccess)

    sender
      .prepareSettings(build, Set(BuildSettingFlag.success))
      .toSet shouldEqual Set.empty[BuildSetting]
  }

  "NotificationSender.prepareSettings" should "not return setting if build fails" in new Context {
    def settingFlags =
      Set(BuildSettingFlag.failure, BuildSettingFlag.successToFailure)

    sender
      .prepareSettings(build, Set(BuildSettingFlag.failure))
      .toSet shouldEqual Set.empty[BuildSetting]
  }
...
object NotificationSenderTest {
  ...
  trait Context extends CommonMocks {
    ...
    val sender =
      new NotificationSenderStub(manager, gateway, messageBuilderFactory)

    def settingFlags: Set[BuildSettingFlag]
    val channelName = "general"
    val setting =
      BuildSetting("buildTypeId", "my-branch", channelName, "", settingFlags)
    ...
    (build.getContainingChanges _)
      .when()
      .returns(
        List
          .empty[SVcsModification]
          .asJava
      )
  }
}
