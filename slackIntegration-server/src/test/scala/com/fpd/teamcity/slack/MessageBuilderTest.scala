...
class SBuildMessageBuilderTest
    extends AnyFlatSpec
    with MockFactory
    with Matchers {
  import SBuildMessageBuilderTest._

  private val buildSetting =
    BuildSetting("", "", "art", "", artifactsMask = ".*")
  ...
  "MessageBuilder.compile" should "compile default template" in {
    ...
    messageBuilder(viewResultsUrl).compile(
      SBuildMessageBuilder.defaultMessage,
      buildSetting
      ...
      """.stripMargin.trim,
      SBuildMessageBuilder.statusNormalColor,
      "✅"
    )
  }
  ...
  "MessageBuilder.compile" should "compile default template with encoded build name" in {
    ...
    (build.getFullName _)
      .when()
      .returns("Deploy -> test-host.io & demo-host.io <-")
    (build.getDuration _).when().returns(100)
    (build.getBuildNumber _).when().returns("2")
    (build.getBranch _).when().returns(branch)
    (branch.getDisplayName _).when().returns("default")
  ...
  "MessageBuilder.compile" should "compile template with artifacts placeholders" in {
    ...
    messageBuilder(downloadArtifactsUrl = downloadUrl)
      .compile(messageTemplate, buildSetting) shouldEqual SlackAttachment(
      s"""Full name
  ...
  "MessageBuilder.compile" should "compile template with artifactsRelUrl placeholders" in {
    ...
    (build.getBuildStatus _).when().returns(Status.NORMAL)
    (build.getContainingChanges _).when().returns(mockChanges)
    (build.getArtifactsDirectory _)
      .when()
      .returns(
        new File(
          "/full/artifacts/path/my/build/folder/"
        )
      )
    ...
    messageBuilder(artifactsPath = "/full/artifacts/path/")
      .compile(messageTemplate, buildSetting) shouldEqual SlackAttachment(
      s"""Full name
        |my/build/folder
      """.stripMargin.trim,
  ...
  "MessageBuilder.compile" should "compile template with parameter placeholders" in {
    ...
    messageBuilder(params = Map(teamcityParam -> teamcityParamValue))
      .compile(messageTemplate, buildSetting) shouldEqual SlackAttachment(
      s"""Full name
        |$teamcityParamValue
      """.stripMargin.trim,
  ...
  "MessageBuilder.compile" should "compile template with artifactLinks placeholder" in {
    ...
    artifactsViewer.iterateArtifacts _ when * onCall {
      processor: BuildArtifacts.BuildArtifactsProcessor =>
  ...
  private def mockChanges = {
    ...
    (vcsModification1.getDescription _)
      .when()
      .returns("Did some changes\nSecond line")
  ...
  private def mockReasons(reasons: List[String]) = reasons
    .map(reason =>
      BuildProblemData.createBuildProblem("identity", "custom", reason)
    )
    .asJava
}
...
object SBuildMessageBuilderTest extends MockFactory {
  ...
  )(implicit build: SBuild): SBuildMessageBuilder = {
    val context = stub[MessageBuilderContext]
    (() => context.getArtifactsPath).when().returns(artifactsPath)
    (() => context.getViewResultsUrl).when().returns(_ => viewResultsUrl)
    (() => context.getDownloadAllArtifactsUrl)
      .when()
      .returns(_ => downloadArtifactsUrl)
    (() => context.userByEmail).when().returns(x => Some(x))
    (() => context.getBuildParameter)
      .when()
      .returns((_, name) => params.get(name))
    (() => context.artifactsPublicUrl).when().returns(Some(artifactsPublicUrl))

    new SBuildMessageBuilder(build, context)
  }

  class SQueuedBuildMessageBuilderTest
      extends AnyFlatSpec
      with MockFactory
      with Matchers {
    ...
    "SQueuedBuildMessageBuilder.compile" should "compile template with name branch status and link placeholder" in {
      ...
      queuedMessageBuilder("Full name", "Branch name", "/some/build/url")
        .compile(messageTemplate, buildSetting) shouldEqual SlackAttachment(
        s"""Full name
           |Branch name
  ...
  object SQueuedBuildMessageBuilderTest extends MockFactory {
    def queuedMessageBuilder(
        queuedBuildName: String,
        queuedBuildBranch: String,
        queuedBuildUrl: String,
        params: Map[String, String] = Map.empty
    )(implicit build: SQueuedBuild): SQueuedBuildMessageBuilder = {

      val context = stub[MessageBuilderContext]

      (() => context.getQueuedBuildName).when().returns(_ => queuedBuildName)
      (() => context.getQueuedBuildBranch)
        .when()
        .returns(_ => queuedBuildBranch)
      (() => context.getQueuedBuildUrl).when().returns(_ => queuedBuildUrl)
  ...
  class TestBuildArtifact(name: String, relativePath: String, file: Boolean)
      extends BuildArtifact {
