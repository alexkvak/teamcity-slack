...
import com.fpd.teamcity.slack.{PermissionManager, Resources, Strings}
...
class BuildPage(
  ...
  override def isAvailable(request: HttpServletRequest): Boolean =
    permissionManager.buildAccessPermitted(
      request,
      getBuildType(request).getInternalId
    )
