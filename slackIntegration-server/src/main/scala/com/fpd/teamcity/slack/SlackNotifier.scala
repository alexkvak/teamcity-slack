...
class SlackNotifier(notificatorRegistry: NotificatorRegistry)
    extends NotificatorAdapter {
  import SlackNotifier._

  notificatorRegistry.register(this)

  override def notifyBuildStarted(
      build: SRunningBuild,
      users: util.Set[SUser]
  ): Unit = ()
...
object SlackNotifier {
  implicit private def propertyNameToPropertyKey(
      propertyName: String
